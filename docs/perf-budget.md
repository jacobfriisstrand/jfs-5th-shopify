...
- **50 KB homepage** — a fully-fledged Shopify homepage typically composes hero, featured product/collection, image-with-text, testimonials, and newsletter sections. 50 KB allows those sections plus the global runtime (header, dialog, polyfills) without pre-emptively shipping PDP-specific code. If the homepage adds quick-add cards (which pull in `product-form.js` + `variant-picker.js`), that's an explicit budget-raise conversation — not a free lunch. If the homepage adds quick-add cards (which pull in `product-form.js` + `variant-picker.js`), that's an explicit budget-raise conversation — not a free lunch.
