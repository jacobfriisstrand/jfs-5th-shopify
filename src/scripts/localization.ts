...
import { isClickedOutside, onAnimationEnd } from "@theme/dom-events";
import { normalizeString } from "@theme/utilities";

/**
 * Toggles `.is-scrolled` on the closest `.country-filter` when the country
 * list scrolls. Shared between the dropdown and drawer wrappers.
 *
 * @this {HTMLElement} The localization wrapper that owns the country filter.
 * @param {Event} event The scroll event.
 */
function handleCountryListScroll(this: Element, event: Event) {
...
class LocalizationFormComponent extends Component {
  ...
  #onCountryListScroll = handleCountryListScroll.bind(this);
}
