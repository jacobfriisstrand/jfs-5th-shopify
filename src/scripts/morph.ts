...
export type MorphOptions = {
  ...
  /** Get node key for matching */
  getNodeKey?: (node: Node | undefined) => string | number | undefined;
  /** Pre-update hook */
  onBeforeUpdate?: (oldNode: Node, newNode: Node) => void;
  /** Post-update hook */
  onAfterUpdate?: (node: Node) => void;
  /** Reject a node from being morphed */
  reject?: (oldNode: Node, newNode: Node) => boolean;
  /** If true, only morph subtrees whose elements have `data-hydration-key="<non-empty>"`, matched by that value */
  hydrationMode?: boolean;
};

const HYDRATION_KEY_ATTRIBUTE = "data-hydration-key";

/**
 * Registry of preserver functions invoked during `onBeforeUpdate`.
 *
 * Custom elements (or any module) that need to keep state across a morph —
 * e.g. a popover whose inline `style` is positioned by Floating UI, or a
 * widget whose `aria-expanded` reflects local UI state not present in the
 * server response — register a function that copies the relevant
 * attribute(s)/style(s) from the old node to the new one.
 *
 * Register at module top-level so the rule is co-located with the element
 * that owns it; there's no de-registration.
 *
 * @example
 ...
 *   });
 */
const morphPreservers: Array<(oldNode: Element, newNode: Element) => void> = [];

export function registerMorphPreserver(
  fn: (oldNode: Element, newNode: Element) => void,
) {
  morphPreservers.push(fn);
}
...
export const MORPH_OPTIONS: MorphOptions = {
  childrenOnly: true,
  hydrationMode: false,
  reject(oldNode: Node, newNode: Node) {
    if (
  ...
  onBeforeUpdate(oldNode: Node, newNode: Node) {
    if (!(oldNode instanceof Element) || !(newNode instanceof Element)) {
      return;
    }

    // Run any preservers registered by Custom elements that need to keep
    // state across a morph. See `registerMorphPreserver` above.
...
export function morph(
  oldTree: Node,
  newTree: Node | string,
  options: MorphOptions = MORPH_OPTIONS,
): Node {
  if (!oldTree || !newTree) {
 ...
 */
function collectHydrationTargets(root: Element): Element[] {
...
function morphHydrationByKey(
  ...
  const oldTargetsByKey = new Map<string, Element[]>();
 ...
 */
function walk(newNode: Node, oldNode: Node, options: MorphOptions): Node {
 ...
 */
function updateNode(newNode: Node, oldNode: Node, options: MorphOptions): void {
 ...
 */
function updateAttribute(newNode: any, oldNode: any, name: string): void {
 ...
 */
function copyAttributes(newNode: Element, oldNode: Element): void {
...
function updateTextarea(
  newNode: HTMLTextAreaElement,
  oldNode: HTMLTextAreaElement,
): void {
  const newValue = newNode.value;
  if (newValue !== oldNode.value) {
    oldNode.value = newValue;
 ...
 */
function recreateAppBlockScripts(container: Element): void {
...
function updateChildren(
  ...
  let oldChild: ChildNode | undefined;
  let newChild: ChildNode | undefined;
  let morphed: Node;
  let oldMatch: ChildNode | null;
  let offset = 0;
 ...
 */
function same(a: Node, b: Node, options: MorphOptions): boolean {
