...
import { yieldToMainThread } from "@theme/scheduling";
import { getViewParameterValue } from "@theme/utilities";
import { ResizeNotifier } from "@theme/viewport";
...
export default class VariantPicker extends Component {
  ...
  connectedCallback() {
    ...
    const fieldsets = (this.refs.fieldsets || []) as HTMLFieldSetElement[];

    fieldsets.forEach((fieldset) => {
      const radios = Array.from(
        fieldset?.querySelectorAll<HTMLInputElement>("input") ?? [],
      );
  ...
  variantChanged(event) {
    ...
    const isOnFeaturedProductSection = Boolean(
  ...
  buildRequestUrl(
    ...
    const SECTION_ID_MAP = {
      ...
      "featured-product-information": this.closest(
        "featured-product-information",
      )?.id,
    };

    const closestSectionId =
      /** @type {keyof typeof SECTION_ID_MAP} | undefined */ (
  ...
  updateVariantPickerCss() {
    ...
    const measurements = fieldsets
      .map((_, index) => this.#getFieldsetMeasurements(index))
      .filter((m) => m !== null);
